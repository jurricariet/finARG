...
update_balres <- function() {
  ...
  cat("Balance resumido (balres). Última actualización: ",ultima_fecha,"\n","🔍 Buscando actualización para:", fecha_siguiente, "\n")
  ...
  if (!descargado || file.info(temp_file)$size < 1000) {
    message("ℹ️ No hay actualiación disponible para", fecha_siguiente, ".")
  ...
  if (any(grepl("<html", tolower(primeras_lineas)))) {
    message("ℹ️ No hay actualización disponible para balres.")
    return(invisible(NULL))
  }
  ...
  if (!any(balres$fecha == fecha_siguiente)) {
    balres_actualizado <- bind_rows(balres, nuevo_df)
    save(balres_actualizado, file = "data/balres.rda", compress = "xz")
