#' Obtiene el balance resumido de entidades por período y entidad
#'
#' Filtra el dataset de balance resumido del BCRA por períodos seleccionados, entidad (por nombre o código) e indicador (por código).
#'
...
#' @param codigo_cuenta Código(s) exacto(s) de la cuenta Opcional.
#' @return Un dataframe filtrado.
#' @export
get_balres <- function(periodos = NULL, entidad = NULL, codigo_entidad = NULL, codigo_cuenta = NULL) {
  ...
  if (!is.null(codigo_cuenta )) {
    df_filtrado <- dplyr::filter(df_filtrado, codigo_de_linea  %in% !!codigo_cuenta )
  }
